...
class Cube2(Environment):
    def __init__(self, intermediate_reward_name="uniform", add_actions=False):
        ...
        # 2x2 solved state, only corners
        self.corner_tiles_to_check = [i for i in range(54) if i % 9 == 0 or i % 9 == 2 or i % 9 == 6 or i % 9 == 8]
    ...
    def next_state(self, states: List[Cube2State], action: int) -> Tuple[List[Cube2State], List[np.float32]]:
        states_np = np.stack([x.colors for x in states], axis=0)
        states_next_np, transition_costs = self._move_np(states_np, action)

        states_next: List[Cube2State] = [Cube2State(x) for x in list(states_next_np)]
    ...
    def is_solved(self, states: List[Cube2State]) -> np.ndarray:
        ...
        # Check only the corners for a 2x2
        is_equal = np.equal(states_np[:, self.corner_tiles_to_check], np.expand_dims(self.goal_colors[self.corner_tiles_to_check], 0))
    ...
    def generate_states(self, num_states: int, backwards_range: Tuple[int, int]) -> Tuple[List[Cube2State], List[int]]:
        """
        ...
        states: List[Cube2State] = [Cube2State(x) for x in list(states_np)]
    ...
    def expand(self, states: List[State]) -> Tuple[List[List[State]], List[np.ndarray]]:
        ...
        for move_idx in range(num_env_moves):
            ...
            for idx in range(len(states)):
                states_exp[idx].append(Cube2State(states_next_np[idx]))
